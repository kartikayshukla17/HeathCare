                ...
                <div className="flex flex-col sm:flex-row gap-4 mb-8 justify-between">
                    ...
                    <div className="flex bg-gray-200 dark:bg-zinc-700 p-1 rounded-lg self-start">
                        <button
                            ...
                            className={`px-4 py-2 rounded-md text-sm font-semibold transition-colors ${view === 'list' ? 'bg-white dark:bg-zinc-600 shadow-sm text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`}
                        >
                            List
                        </button>
                        <button
                            onClick={() => setView('calendar')}
                            className={`px-4 py-2 rounded-md text-sm font-semibold transition-colors ${view === 'calendar' ? 'bg-white dark:bg-zinc-600 shadow-sm text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`}
                        >
                            Calendar
                        </button>
                    </div>
                    ...
                    {view === 'list' && (
                        <div className="flex bg-white dark:bg-zinc-800 rounded-2xl shadow-sm border border-gray-100 dark:border-zinc-700 p-1 flex-1 sm:max-w-md">
                            <button
                                ...
                                className={`flex-1 py-2 rounded-xl text-sm font-semibold transition-all ${activeTab === 'upcoming' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-zinc-700'}`}
                            >
                                Upcoming
                            </button>
                            <button
                                onClick={() => setActiveTab('history')}
                                className={`flex-1 py-2 rounded-xl text-sm font-semibold transition-all ${activeTab === 'history' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-zinc-700'}`}
                            >
                                History
                            </button>
                        </div>
